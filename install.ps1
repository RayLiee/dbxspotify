...
function Get-File
{
    ...
    else
    {
        ...
        Write-Progress -Activity "Le telechargement du fichier est termin� (ton pc est toujours en vie tkt) '$downloadedFileName'"
...
If ($dbx -eq 'y') 
{ 
   Write-Host "Carr� ! Je te Stop fiveM avant de commencer...`n"
   Stop-Process -Name FiveM
}
else
{
       Write-Host "Carr� ! Je te re Stop Spotify avant de commencer...`n"
       Stop-Process -Name Spotify
       Stop-Process -Name SpotifyWebHelper
}
...
try
{
  ...
  New-Item -Type Directory -Name "D_Spotify-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" |
  Convert-Path |
  Set-Location
}
catch
