...
if ($spotifyInstalled)
{
  $ch = Read-Host -Prompt '(Facultatif) - Mettre a jour Spotify a la derniere version. (Peut etre ce dernier est deja mis a jour). (Y/N)'
  if ($ch -eq 'y')
  {
    $update = $true
  }
  ...
  {
    Write-Host 'Essaye pas de mettre a jour Spotify hein soit pas con.'
  }
}
else
{
