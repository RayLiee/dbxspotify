...
if ($spotifyInstalled)
{
  $ch = Read-Host -Prompt 'Facultatif - Mettre a jour Spotify a la derniere version. (Peut etre deja mis a jour). (Y/N)'
  if ($ch -eq 'y')
  {
    $update = $true
  }
  ...
  {
    Write-Host 'Essaye pas de mettre a jour Spotify.'
  }
}
else
{
...
{
  Write-Host 'Telechargement de la derniere version du exe complete de Spotify, veuillez patienter....'
...
$ch = Read-Host -Prompt 'Facultatif - Suppression de l"emplacement de la publicite et du bouton de mise a niveau. (Y/N)'
if ($ch -eq 'y')
{
...
{
  Write-Host "Impossible de supprimer l'espace publicitaire et le bouton de mise a niveau..`n"
}
