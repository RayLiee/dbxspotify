...
function Get-File
{
    ...
    else
    {
        ...
        while ($count -gt 0)
        {
            ...
            Write-Progress -Activity "Téléchargement du fichier '$downloadedFileName'" -Status "Finish ! ($([System.Math]::Floor($downloadedBytes/1024))K of $($totalLength)K): " -PercentComplete ((([System.Math]::Floor($downloadedBytes / 1024)) / $totalLength) * 100)
        }

        Write-Progress -Activity "Le téléchargement du fichier est terminé (ton pc est toujours en vie tkt) '$downloadedFileName'"
...
@D_BOT message:
Version 3.2.1
Reprise et récréation du projet Open Source de 'Mpond' Pour un meilleur code et des meilleurs fonctionnalité
Usage de ce logiciel strictement privée ! Suppression instantané en cas de leak :)
*****************
'@
...
*****************
Authors: @Doublox
Contributeurs: @Nuzair46, @KUTlime (Pour le système de téléchargement du Spotify et de la désinstallation si ce dernier vien de Microsoft )
*****************
'@
...
Write-Host "Je te Stop spotify avant de commencer...`n"
...
{
  Write-Host "La version Microsoft Store de Spotify a été détectée et n'est pas prise en charge..`n"

  $ch = Read-Host -Prompt 'Je peux te le désinstaller ? (Y/N)'
  if ($ch -eq 'y')
  {
    Write-Host "Désinstallation de Spotify en cours...`n"
...
Write-Host "Téléchargement du dernier patch (chrome_elf.zip)...`n"
...
if ($spotifyInstalled)
{
  $ch = Read-Host -Prompt 'Facultatif - Mettre à jour Spotify à la dernière version. (Peut être déjà mis à jour). (Y/N)'
  if ($ch -eq 'y')
  {
    $update = $true
  }
  ...
  {
    Write-Host 'Essaye pas de mettre à jour Spotify.'
  }
}
else
{
  Write-Host 'L"Installation de Spotify a pas été détectée.'
}
if (-not $spotifyInstalled -or $update)
{
  Write-Host 'Téléchargement de la dernière version du exe complète de Spotify, veuillez patienter....'
  ...
  if ($isUserAdmin)
  {
    ...
    Write-Host 'Désenregistrement de la tâche...'
...
$ch = Read-Host -Prompt 'Facultatif - Suppression de l"emplacement de la publicité et du bouton de mise à niveau. (Y/N)'
if ($ch -eq 'y')
{
  ...
  elseif (Test-Path $xpuiUnpackedPath)
  {
    ...
    Write-Host 'Spicetify détecté - Il se peut que vous deviez réinstaller le BTS après avoir exécuté "spicetify apply".';
  }
  else
  {
...
{
  Write-Host "Impossible de supprimer l'espace publicitaire et le bouton de mise à niveau..`n"
}
...
Write-Host 'Patching terminé, démarrage de Spotify...'
